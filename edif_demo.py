...
chack_img_name = "Places365_val_00003852"
edit_prompt = 'make it spring scene'
edit_keyword = 'spring'
...
def main(args):
    ...
    def onetime_inference(self, latents, image_embeds, image_latents, t, i, guidance,
        ...
        def predict_x0(start_noise, noise_pred, present_latent, do_save=True, do_newname=False):
            ...
            return x0_pred_latent, x0_pred_pil, noise_pred_pil, present_latent_pil

        x0_pred_latent, x0_pred_pil, noise_pred_pil, present_latent_pil = predict_x0(start_noise, noise_pred, latents,
                                                                                     do_save=True)

        # ------------------------------------------------------------------------------------
        def set_compare_tensor(inference_step, noise_pred, x0_pred):
            if inference_step > -1:
                if save_time > 300:
                    compare_tensor = x0_pred
                    compare_pil = x0_pred_pil
                else:
                    compare_tensor = noise_pred
                    compare_pil = present_latent_pil
            return compare_tensor, compare_pil

        compare_tensor, compare_pil = set_compare_tensor(timestep, noise_pred, x0_pred_latent)
        ...
        print(f' [{save_time}] structure_similarity: {structure_similarity}')
        # structure_ideal_low = 0.3 structure_ideal_high = 0.9
        ...
        if structure_similarity < ideal_structure_similar_range[0]:
            print(f' Use New Noise Pred!')#
            # noise_pred = start_noise - image_latents
            from diffusers.training_utils import compute_density_for_timestep_sampling
            def get_sigmas(i, n_dim=4, dtype=torch.float32):
                sigmas = self.scheduler.sigmas.to(device=image_latents.device, dtype=dtype)
                sigma = sigmas[i].flatten()
                while len(sigma.shape) < n_dim:
                    sigma = sigma.unsqueeze(-1)
                return sigma
            sigmas = get_sigmas(i,n_dim=image_latents.ndim,dtype=image_latents.dtype)
            latents = (1.0 - sigmas) * image_latents + sigmas * start_noise
            # -------------
            # make noisy_latents
            # -------------
            latent_model_input = torch.cat([latents, image_latents], dim=1)
            ...
            noise_pred = self.transformer(
                ...
                return_dict=False, )[0]
            noise_pred = noise_pred[:, : latents.size(1)]
            x0_pred_latent, x0_pred_pil, noise_pred_pil, present_latent_pil = predict_x0(start_noise, noise_pred,
                                                                                         latents,
                                                                                         do_save=True,do_newname=True )
            compare_tensor, compare_pil = set_compare_tensor(timestep, noise_pred, x0_pred_latent)
            compare_pil.save(os.path.join(inter_folder, f'comparison_{save_time}.png'))
            structure_similarity = get_structural_similarity(compare_pil, input_image)
            print(f' [{save_time}] new structure_similarity: {structure_similarity}')

        # -------------------------------------------------------------------------------------
        # Compare Again
        # -------------------------------------------------------------------------------------
        delta_img = 0.3
        print(f'[⚠] structure_similarity {structure_similarity} is under ideal ({ideal_structure_similar_range[0]}) → image ↑')
        if structure_similarity > ideal_structure_similar_range[1]: # too similary
            delta_img = 0.2
            print(
            ...
            for k in controller.img_alpha_dict:
                ...
                if k == 0:
                    print(f'  [↓] img_alpha[{k}]: {old_img_alpha:.2f} → {new_img_alpha:.2f} | {action}')
        """
        else:
            # 구조는 유사 범위이긴 한데, 겨우 간단단당인 상황
            # txt 를 조금 낮추기
            delta_txt = 0.1
            for k in controller.txt_alpha_dict:
                ...
                if k == 0:
                    print(f"  [↑] txt_alpha[{k}]: {old_txt_alpha:.2f} → {new_txt_alpha:.2f} | {action}")
        """
        # ------------------------------------------------------------------------------------------------------------------------------
            ...
            conversation = [
                {
                    ...
                    "content": [
                        ...
                        {"type": "text", "text": f"이 이미지에 {user_edit_keyword} 가 잘 표현되었니?"
                                                 f"잘 표현되었따면 Yes 로 답하고 아니면 No 로 답해줘. 다른 답은 말고, Yes 와 No 로만 답해줌"},
                    ],
                }
            ]
        ...
        for attempt in range(max_retry):
            ...
            if txt_decision.lower() in ['yes', 'no']:
                ...
                print(f"[{attempt + 1}/{max_retry}] Unexpected response: '{txt_decision}' → retrying...")
        print(f' VLM txt_decision = {txt_decision}')

        if txt_decision.lower() not in ['yes', 'no']:
            print(f"[✘] Failed to get valid decision after {max_retry} attempts: '{txt_decision}'")
            # 여기서 fallback 로직이나 예외처리 가능

        delta_txt = 0.2  # 조정 범위
        if txt_decision.strip().lower() == 'no':
            print(f"[✘] {user_edit_keyword} 가 잘 표현되지 않음 → 텍스트 알파 ↑")
            for k in controller.txt_alpha_dict:
                old_txt_alpha = controller.txt_alpha_dict[k]
                if k in QTB:
                    new_txt_alpha = min(2.0, old_txt_alpha + delta_txt * 0.5)
            ...
            print(f"[✓] '{user_edit_keyword}' 가 잘 표현됨 → txt 그대로 유지 + start latent change")

        else:
            print(f"[!] 텍스트 인식 실패 또는 이상 응답: {txt_decision.strip()} → check_text_editing() 재시도 또는 예외 처리 필요")
        ...
        print(f'# 첫번째 img alpha dict: key = {first_img_key}, value = {img_alpha_dict[first_img_key]}')
        print(f'# 첫번째 txt alpha dict: key = {first_txt_key}, value = {txt_alpha_dict[first_txt_key]}')
    ...
    save_folder_base = '/data2/CVPR_Public/Kontext_EDIF_new_20250726_onesample'
    ...
    # 13 번
    # 352 번
    image_folder = './datas/imageedit/demo'
    edit_file = './datas/imageedit/demo/demo.json'
    with open(edit_file, 'r') as f :
        contents = f.########

    for content in contents:
        name = content['filename']
        edit_prompt = content['scene_change'][0]
        image_path = os.path.join(image_folder, name)
        save_name, _ = os.path.splitext(name)

        save_dir = os.path.join(save_folder_base, f'{save_name}_{edit_keyword}_ideal_low_{args.structure_ideal_low}_ideal_high_{args.structure_ideal_high}')
        ...
        def get_source_caption() :
            conversation = [
                {
                    "role": "user",
                    "content": [
                        {"type": "image", "path": image_path},
                        {"type": "text", "text": "Explain thie image into one sentence"},
                    ],
                }
            ]
            inputs = processor.apply_chat_template(
            ...
            return output_text
        #source_caption = get_source_caption()[0]
