...
class FluxKontextPipeline(
    ...
    def __call__(
        ...
        num_images_per_prompt: Optional[int] = 1,
            structure_ideal_low: Optional[float] = None,
            structure_ideal_high: Optional[float] = None,
        generator: Optional[Union[torch.Generator, List[torch.Generator]]] = None,
            ...
            for i, t in enumerate(timesteps):
                ...
                if original :
                    ...
                    if save_dir is not None :
                        latents_ = self._unpack_latents(latents, height, width, self.vae_scale_factor)
                        latents_ = (latents_ / self.vae.config.scaling_factor) + self.vae.config.shift_factor
                        image_ = self.vae.decode(latents_, return_dict=False)[0]
                        image_ = self.image_processor.postprocess(image_, output_type=output_type)
                        image_[0].save(os.path.join(save_dir, str(i).zfill(6) + ".png"))

                        # x0_predict
                        def predict_x0(start_noise, noise_pred):
                            ...
                            return x0_pred_latent, x0_pred_pil

                        x0_pred_latent, x0_pred_pil = predict_x0(start_noise, noise_pred)
                        x0_pred_pil.save(os.path.join(save_dir,f'x0_pred_{str(i).zfill(6)}.png'))

                else :
                    latents, image_latents,change_num = self.onetime_inference(latents=latents,
                                                     ...
                                                     structure_ideal_high=structure_ideal_high,change_num=change_num)
                    print(f' updated latent')
        self._current_timestep = None
