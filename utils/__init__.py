...
def log_validation(flux_transformer, args, accelerator, weight_dtype, step, wandb_obj, is_final_validation=False, pipeline = None):
    logger.info("Running validation... ")

    if not is_final_validation:
        flux_transformer = accelerator.unwrap_model(flux_transformer)
    else:
        flux_transformer = FluxTransformer2DModel.from_pretrained(args.output_dir, torch_dtype=weight_dtype)


    print(f' Start Loading Pipe')
    from model.pipeline_select import FluxKontextPipeline
    #pipeline = FluxKontextPipeline.from_pretrained("black-forest-labs/FLUX.1-Kontext-dev",
    #                                               torch_dtype=torch.bfloat16,
    #                                               transformer=flux_transformer,
    #                                               cache_dir='/workspace/model/FluxKontext')
    pipeline.transformer = flux_transformer
    pipeline.to(accelerator.device)
    pipeline.set_progress_bar_config(disable=True)
    print(f' Finish Loading Pipe')
        ...
        for gs in args.validation_guidance_scale:
            ...
            for _ in range(args.num_validation_images):
                with autocast_ctx:
                    ...
                    # wandb log image
                    # ---------------------
                    wandb_obj.log({"generated_image": [wandb.Image(image,
                                                               caption=f"Guidance: {gs}")]})
                images.append(image)
            image_logs.append(
                {
    ...
    for tracker in accelerator.trackers:
        if tracker.name == "wandb":
            ...
            for log in image_logs:
                ...
                formatted_images.append(wandb.Image(validation_image, caption=f"source image"))
                for image in images:
                    image = wandb.Image(image, caption=f"generated [{validation_prompt}]")
